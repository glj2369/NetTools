...
public class IPAddressUtils {
    ...
    public void init() {
        try {
            ...
            b3 = new byte[3];
            try {
                String path = Environment.getExternalStorageDirectory().getPath()+ "/NetToolDate/ipdata/qqwry.dat";
                File file=new File(path);
                ipFile = new RandomAccessFile(file,"r");
                //ipFile = context.getResources().openRawResource(R.raw.qqwry);
    ...
    public String getIp(Context context, String address) {
        this.context = context;

        IPAddressUtils ip = new IPAddressUtils();
        ip.init();//System.out.println("android.resource://" +context .getApplicationContext().getPackageName() + "/" +"raw/qqwry.dat");
        //address = "112.225.35.70";
        //return "IP地址[" + address + "]获取到的区域信息:" + ip.getIPLocation(address).getCountry() + ", 获取到的城市:" + ip.getIPLocation(address).getCity() + ", 运营商:" + ip.getIPLocation(address).getArea();
        return ip.getIPLocation(address).getCountry() + " "+ip.getIPLocation(address).getArea();
    }
